...
unsigned char Fout_data_len[2]={0x00,0x17};//四面出风上报数据长度
...
//四面出风数据封装函数
void APP_PROCESS_DEVICE(u8 buf[],u8 len){
	u8 data_len;
	u8 uart3_databuf[USART_REC_LEN];
	mymemcpy(uart3_databuf,buf,len);
	data_len=len;
	//保存添加PM2.5数据
//	if(data_len==22){
//			Four_data_up_t four_data_up_t;//加入PM2.5数据的结构体,总长22
//			uint16_t check_data;
//			unsigned char check_buf[2];
//			mymemcpy(four_data_up_t.data_buf,uart3_databuf,18);//一共22个字节，减去包尾和校验和，还剩余18个字节
//		  mymemcpy(four_data_up_t.data_core.Data_length,Fout_data_len,2);
//			mymemcpy(four_data_up_t.data_core.PM25,PM25_DATA,3);
//		
//		  check_data=Sum_data(four_data_up_t.data_buf,2,20);
//			check_buf[0]=check_data>>8;
//			check_buf[1]=check_data;
//			mymemcpy(four_data_up_t.data_core.Check_code,check_buf,2);
//		
//		  mymemcpy(four_data_up_t.data_core.Tial,TAIL_DATA,2);
//			myuart_send(HAL_UART2,four_data_up_t.data_buf,25);
//	}else{
//			myuart_send(HAL_UART2,uart3_databuf,data_len);
//		}
			myuart_send(HAL_UART2,uart3_databuf,data_len);
			//清空数据	
			data_len=0;
			mymemset(uart3_databuf,0,USART_REC_LEN);
}
...
//				}
			...
			//做透传解析
			if(uart2_commandbuf[0]==0xEB&uart2_commandbuf[1]==0x90){
			myuart_send(HAL_UART3,uart2_commandbuf,uart2_len);
			}
			USART2_RX_STA=0;
			uart2_len=0;
			...
			APP_PROCESS_DEVICE(uart3_commandbuf,uart3_len);
			
			USART3_RX_STA=0;
			mymemset(USART3_RX_BUF,0,USART_REC_LEN);
